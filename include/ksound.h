...
#if !defined(DSOUND)  // WAVEFORMATEX Could already be defined by dsound.h
...
void umixerkill (long i)
{
    ...
    umixernum--;
    if (i != umixernum) umixer[i] = umixer[umixernum];
}
...
static void rendersamps (long dasnd, long ispos, long isinc, long ivolsc, long ivolsci, long *lptr, long nsamp, short *coefilt)
{
    ...
    i = isinc*nsamp + ispos;
    if (i < isinc)              return; //ispos <  0 for all samples!
    j = *(long *)(dasnd-8);
    if ((ispos>>LSAMPREC) >= j) return; //ispos >= j for all samples!
    //Clip off low ispos
    if (ispos < 0)
    {
        k = (isinc-1-ispos)/isinc;
        ivolsc += ivolsci*k;
        ispos += isinc*k;
        lptr += k*2;
        nsamp -= k;
...
{
    void *w[2];
    unsigned long l[2], playcurs, writcurs, leng;
	unsigned long i;
    long j, k, m, n, *lptr[2], nsinc0, nsinc1, volsci0, volsci1;
    ...
    leng = ((playcurs-oplaycurs)&(SNDSTREAMSIZ-1));
    if ((long)leng < minleng) return;
    ...
    else
    {
        ...
        for(j=numrendersnd-1; j>=0; j--)
        {
            ...
            if (rendersnd[j].flags&KSND_3D)
            {
                ...
                if (f <= SNDMINDIST*SNDMINDIST)
                {
                    f = SNDMINDIST;
                    h = (float)rendersnd[j].ivolsc;
                }
                else
                {
                    f = sqrt(f);
                    h = ((float)rendersnd[j].ivolsc)*SNDMINDIST/f;
                }
                ...
                h = MAX((f-SNDMINDIST)+g*(EARSEPARATION*.5f),0);
                nsinc0 = k - mulshr16((long)(h*(65536.f/SNDSPEED)),m);
                h = MAX((f-SNDMINDIST)-g*(EARSEPARATION*.5f),0);
                nsinc1 = k - mulshr16((long)(h*(65536.f/SNDSPEED)),m);
                ...
                if (w[m])
                {
                    ...
                    else
                    {
                        ...
                        while ((rendersnd[j].ispos0 >= numsamps) && (rendersnd[j].ispos1 >= numsamps))
                        {
                            ...
                            rendersnd[j].ispos0 -= repleng;
                            rendersnd[j].ispos1 -= repleng;
                        }
                    }
                }
...
{
    WAVEFORMATEX wft;
    long i, j, leng, hashind, newsnd, repstart;
    ...
    else
    {
        ...
        if (*(long *)&tempbuf[0] != 0x46464952)
        {
            fclose(fil);    //RIFF
            return(0);
        }
        if (*(long *)&tempbuf[8] != 0x45564157)
        {
            fclose(fil);    //WAVE
            return(0);
        }
        for(j=16; j; j--)
        {
            ...
            {
                fread(&wft,16,1,fil);
                if ((wft.wFormatTag != WAVE_FORMAT_PCM) || (wft.nChannels != 1))
                {
                    fclose(fil);
                    return(0);
                }
    ...
    if (wft.wBitsPerSample == 16) i = leng;
    else i = (leng<<1); //Convert 8-bit to 16-bit
    ...
    *(long *)(newsnd+leng+16) = *(long *)(newsnd+leng+20) = 0;
    ...
    j = 12+strlen(filnam)+1;
    if (!audcheckhashsiz(j)) return(0);
    ...
    *(long *)&audhashbuf[audhashpos+8] = newsnd;
    ...
    audhashead[hashind] = audhashpos;
    audlastloadedwav = audhashpos;
    audhashpos += j;
...
void playsound (const char *filnam, long volperc, float frqmul, void *pos, long flags)
{
    ...
    long i, j, m, ispos, ispos0, ispos1, isinc, ivolsc, ivolsc0, ivolsc1, newsnd, *lptr[2], numsamps;
    short *coefilt;
    float f, g, h;
    ...
    if (flags&KSND_MEM)
    {
        ...
        if (!(newsnd = audgetfilebufptr(tempbuf)))
        {
            LEAVEMUTX;
            return;
        }
    ...
    {
        if (!(newsnd = audgetfilebufptr(filnam)))
        {
            LEAVEMUTX;
            return;
        }
    ...
    ispos = 0;
    isinc = (long)((*(float *)(newsnd-4))*frqmul*rsamplerate);
    ivolsc = (volperc<<15)/100;
    if (ivolsc >= 32768) ivolsc = 32767;
    ivolsc <<= 16;
    ...
    if (!(flags&KSND_LOPASS)) coefilt = coef;
    else coefilt = coeflopass;
    ...
    if (flags&KSND_3D)
    {
        ...
        m = isinc*samplerate;
        h = MAX((f-SNDMINDIST)+g*(EARSEPARATION*.5f),0);
        ispos0 = -mulshr16((long)(h*(65536.f/SNDSPEED)),m);
        h = MAX((f-SNDMINDIST)-g*(EARSEPARATION*.5f),0);
        ispos1 = -mulshr16((long)(h*(65536.f/SNDSPEED)),m);
    }
    else
    {
        ispos0 = ispos1 = 0;
        ivolsc0 = ivolsc1 = ivolsc;
    }

    kensoundbreath(SNDSTREAMSIZ>>1); //Not necessary, but for good luck
    if (KSoundBuffer_GetCurrentPosition(streambuf,&playcurs,&writcurs) != KS_OK)
    {
        LEAVEMUTX;
        return;
    }
    ...
    if (KSoundBuffer_Lock(streambuf,i,(oplaycurs-i)&(SNDSTREAMSIZ-1),&w[0],&l[0],&w[1],&l[1],0) != KS_OK)
    {
        LEAVEMUTX;
        return;
    }
    ...
    if ((((ispos0>>LSAMPREC) < numsamps) || ((ispos1>>LSAMPREC) < numsamps) || (flags&KSND_LOOP)) && (numrendersnd < MAXPLAYINGSNDS))
    {
        ...
        {
            rendersnd[numrendersnd].p.x = rendersnd[numrendersnd].p.y = rendersnd[numrendersnd].p.z = 0;
        }
...
void playsoundupdate (void *optr, void *nptr)
{
    ...
    for(i=numrendersnd-1; i>=0; i--)
    {
        ...
        if (((long)nptr) == -2)
        {
            rendersnd[i].flags |= KSND_LOPASS;
            rendersnd[i].coefilt = coeflopass;
            continue;
        }
        if (((long)nptr) == -3)
        {
            rendersnd[i].flags &= ~KSND_LOPASS;
            rendersnd[i].coefilt = coef;
            continue;
        }
